...
class BitmexClient(BaseClient):
    ...
    def __init__(self, keys, leverage, alert_id = None, alert_token = None, markets_list=[], max_pos_part=20):
        ...
        self.subscriptions = ['margin', 'order', 'position', 'orderBook10', 'execution']
        ...
        self.commission = self.swagger_client.User.User_getCommission().result()
    ...
    def get_all_instruments(self):
        ...
        instr_list = {}
        instruments = self.swagger_client.Instrument.Instrument_get(filter=json.dumps({'quoteCurrency': 'USDT', 'state': 'Open'})).result()
                    ...
                    if table == 'orderBook10':
                        for ob in message['data']:
                            if ob.get('symbol'):
                                ...
                                symbol = ob['symbol']
                                self.data[table].update({symbol: ob})
                    else:
